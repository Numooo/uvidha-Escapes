...
export function HolidaysPage({ onPackageSelect }: HolidaysPageProps) {
  ...
  const themeKeys = [
    "all", "beach", "adventure", "heritage", "wellness", "mountains", "wildlife", "honeymoon", "family"
  ];
  
  const themes = useMemo(() => themeKeys.map(key => ({
...
function PackageCard({ package: pkg, index, onSelect }: PackageCardProps) {
  ...
  const [isFavorite, setIsFavorite] = useState(false);
   const packageImage = pkg.images?.[0] || pkg.image || undefined;
